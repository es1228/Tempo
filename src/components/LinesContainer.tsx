	...
	return (
		<div className="bg-on-bg-secondary dark:bg-on-bg-dark-secondary w-fit space-y-2 rounded-3xl p-4">
			...
			<div className="block max-w-full overflow-auto lg:max-w-65">
				<p
					className="text-sm h-fit w-fit float-left mr-2 rounded-3xl p-2"
					style={
						score1.at(0) === "+"
				...
				<p className="text-justify text-wrap text-sm">{convertLineToSan(pv[0]?.moves, chessPosition)}</p>
			</div>
            <hr className="my-2 rounded" />
			<div className="block max-w-full overflow-auto lg:max-w-65">
				<p
				...
				<p className="text-justify text-wrap text-sm">{convertLineToSan(pv[1]?.moves, chessPosition)}</p>
			</div>
		</div>
	);
};
