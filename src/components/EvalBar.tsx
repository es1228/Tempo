...
const EvalBar = ({ evaluation, isFlipped }: EvalBarProps) => {
	...
	return (
		...
		>
			...
			>
				{winProbability < 0.5 && (
					<p className="p-2 text-center text-xs text-white" style={{rotate: `${isFlipped ? "180deg" : ""}`}}>
						{evalFormatted}
					</p>
				)}
			</div>
			{winProbability >= 0.5 && (
				<p className="mt-auto p-2 text-xs text-center text-black" style={{rotate: `${isFlipped ? "180deg" : ""}`}}>
					{evalFormatted}
				</p>
			)}
		</div>
