...
const ImportDialog = ({
	...
	return (
		<div
			className={`bg-on-bg dark:bg-on-bg-dark fixed top-1/2 left-1/2 z-10000 mt-20 h-screen w-screen -translate-x-1/2 -translate-y-1/2 space-y-4 overflow-auto p-8 transition-all duration-300 ease-in-out lg:mt-0 lg:h-2/3 lg:w-1/2 lg:rounded-3xl ${isDialogOpen ? "pointer-events-auto visible scale-100 transform opacity-100" : "pointer-events-none invisible scale-95 transform opacity-0"} `}
			ref={clickRef}
		>
			...
			{selectedValue === "Chess.com" && (
				<>
					<div className="flex flex-row items-center justify-between">
						<Button
							onClick={() =>
								month < range && setMonth((prev) => prev + 1)
							}
							icon="arrow_back"
						/>
						<p>
					...
					<ul className="mb-50 space-y-4 lg:mb-0">
						{games.map((game) => (
							<ChessGameItem
								key={game.url}
